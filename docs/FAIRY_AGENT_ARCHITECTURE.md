...
17. Model health 只能由已校验的固定字段 Trace 聚合；观测结果不得反向改变路由、行为经验或自动学习状态。
18. Recent failures 只能投影固定字段且最多保留 20 条；任何非法存量字段必须 fail closed，不能通过失败详情恢复用户、会话、正文、Prompt、工具参数或内部 Trace 标识。
19. 配置 revision 只有在对应运行快照实际生效后才能成为 active revision；审计只允许固定分类，不能记录配置值或身份信息。
