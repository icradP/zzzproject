...
| M4 | 语音消息、转发与链接分享、定位、戳一戳 | 已完成，待生产验证 | 完善日常通信能力 |
...
2. 完全屏蔽时是否仍累计未读数，以及群公告是否例外。
3. 系统管理员能否查看私聊内容，以及相关审计与隐私边界。
4. Fairy 是否默认开放、是否按群单独启用，以及 AI 调用额度如何限制。
5. 音视频房间的最大人数、清晰度和月度带宽预算。
