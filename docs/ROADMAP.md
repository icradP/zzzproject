...
| 里程碑 | 内容 | 原因 |
| --- | --- | --- |
| M1 | PWA 加载反馈、性能监控、缓存优化 | 当前最直接影响访问和留存的问题 |
| M2 | 双媒体通道、Hash 去重、缩略图、压缩 | 为语音、名片背景和后续媒体能力打基础 |
| M3 | 更新说明、内置表情、来源标识简化 | 风险低，可以快速改善使用体验 |
| M4 | 语音消息、转发与链接分享、定位、戳一戳 | 完善日常通信能力 |
| M5 | 管理员角色、群公告、屏蔽策略 | 建立群组治理和通知规则 |
| M6 | 称号和个人名片 | 在权限与媒体能力稳定后扩展个人表达 |
| M7 | Fairy AI 好友与 ZZZ 插件 | 以独立 Bot 服务接入，控制对核心 IM 的影响 |
| M8 | 语音房间、视频和直播房间 | 成本与技术复杂度最高，最后建设 |
