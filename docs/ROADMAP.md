...
当前进度（M7 Agent 化 F5.17，显式生产 AI 启停，本地实现、待发布）：
...
- 自动行为学习保持关闭，生产 MiMo 仍未配置或启用，M8 继续暂停。
...
| M7 | Fairy AI 好友与 ZZZ 插件 | Agent F0-F5.16 已部署，F5.17 待发布；生产 AI 待配置 | 以独立 Bot 服务接入，控制对核心 IM 的影响 |
