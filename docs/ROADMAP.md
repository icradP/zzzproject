...
- 聊天媒体支持服务端托管和客户端自定义图床两种模式；个人名片背景只使用客户端图床或已有网络图片 URL。
...
- 背景支持直接填写 HTTPS 图片地址，或选择本地图片后由客户端压缩为最长 `1600×900`、质量 82 的 JPEG，再直传到客户端配置的图床。
- IM 服务端只校验并保存图床返回的最终 HTTPS URL，不接收、下载、代理或保存名片背景图片二进制；接收端只按普通网络图片预览。
...
| M6 | 称号和个人名片 | 实现完成，待上线验收 | 在权限与媒体能力稳定后扩展个人表达 |
...
13. 个人名片背景与聊天图片的普通客户端图床配置共用；服务端只保存最终 HTTPS URL，接收端仅按网络图片显示。

### 待确认

1. 服务端托管媒体的保留期限、单文件大小限制和用户存储配额。
2. Fairy 是否默认开放、是否按群单独启用，以及 AI 调用额度如何限制。
